...
export function Dashboard() {
    ...
    return (
        <main className="relative isolate h-dvh min-h-150 overflow-hidden bg-background" onClick={closeAllMenus}>
            ...
            <header
                className="pointer-events-none absolute inset-x-0 top-0 z-30 flex items-start justify-between gap-3 p-3 md:p-5">
                <div
                    className="glass pointer-events-auto flex h-14 items-center gap-3 rounded-2xl px-3 shadow-glass md:h-16 md:px-4">
                    <div
                    ...
                    <label className="hidden h-10 min-w-64 items-center gap-2 rounded-xl bg-secondary/80 px-3 md:flex">
                        ...
                        <input className="w-full bg-transparent text-sm outline-none placeholder:text-muted-foreground"
                               placeholder="Buscar sector o dirección"/>
                    </label>
                </div>
                ...
                <div className="pointer-events-auto relative" onClick={(event) => event.stopPropagation()}>
                    <div className="glass flex h-14 items-center gap-1 rounded-2xl p-2 shadow-glass md:h-16">
                        ...
                        >
                            ...
                            {hasUnreadNotifications && (
                                <span
                                    className="absolute top-1.5 right-1.5 size-2.5 rounded-full border-2 border-card bg-destructive shadow-sm animate-pulse"/>
                            )}
                        </Button>
                    ...
                    {getInitials(user.nombre)}
                                ...
                                <ChevronDown data-icon="inline-end"
                                             className={cn('hidden transition-transform sm:block', accountOpen && 'rotate-180')}/>
                            </Button>
                        ) : (
                        ...
                        >
                            ...
                            {notifications.length > 0 ? (
                                <div className="flex max-h-72 flex-col gap-1.5 overflow-y-auto overflow-x-hidden pr-1">
                                    {notifications.map((n) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                <div
                                                    className={`mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-xl ${n.color}`}>
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-center justify-between gap-2">
                                                        ...
                                                        <div className="flex items-center gap-1.5 shrink-0">
                                                            <span
                                                                className="text-[10px] text-muted-foreground">{n.time}</span>
                                                            <button
                                                                type="button"
                                ...
                                <div className="flex flex-col items-center justify-center p-6 text-center">
                                    ...
                                    <p className="mt-2 text-xs font-semibold text-muted-foreground">No tienes
                                        notificaciones pendientes</p>
                                </div>
                            )}
                        </div>
                    )}
                    ...
                    {accountOpen && user && (
                        <div
                            role="menu"
                            className="absolute right-0 top-[calc(100%+0.5rem)] flex w-56 flex-col gap-1 rounded-2xl border bg-card/95 p-2 shadow-xl backdrop-blur-md animate-dropdown"
                        >
                            ...
                            <button role="menuitem" onClick={logout}
                                    className="flex items-center gap-2 rounded-xl px-3 py-2 text-left text-sm text-destructive hover:bg-destructive/10">
            ...
            <aside onClick={(e) => e.stopPropagation()}
                   className={cn('absolute bottom-24 left-3 top-24 z-20 flex w-[calc(100%-1.5rem)] max-w-80 flex-col rounded-3xl border bg-card/95 p-4 shadow-lg transition-all duration-300 md:bottom-5 md:left-5 md:top-28', panelOpen ? 'translate-x-0 opacity-100 pointer-events-auto' : '-translate-x-[120%] opacity-0 pointer-events-none')}>
                ...
                <div className="mt-5 flex items-center justify-between gap-2">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => void fetchReports()}><SlidersHorizontal
                        data-icon="inline-start"/>Actualizar</Button>
                </div>
                <div className="mt-2 flex gap-1 rounded-xl bg-secondary/70 p-1">
                    {filters.map((filter) => (
                        <button key={filter} onClick={() => setActiveFilter(filter)}
                                className={cn('flex-1 rounded-lg px-2 py-2 text-xs font-medium transition-colors', activeFilter === filter ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground')}>
                            {filter}
                        </button>
                    ))}
                </div>
                ...
                <div className="mt-5 flex items-center justify-between gap-3">
                    ...
                    <span
                        className="text-xs text-muted-foreground">{loading ? 'Cargando...' : `${filteredReports.length} reportes`}</span>
                </div>
                <div className="mt-3 flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto pr-1">
                    ...
                    {!loading && filteredReports.length === 0 && (
                        <p className="p-4 text-center text-xs text-muted-foreground">No hay reportes disponibles en este
                            filtro.</p>
                    )}
                </div>
            ...
            <div className="absolute bottom-24 right-3 z-20 flex flex-col gap-2 md:bottom-6 md:right-5"
                ...
                <Button variant="outline" size="icon-lg" className="rounded-2xl bg-card shadow-md hover:bg-secondary"
                        onClick={() => {
                            handleLocateUser();
                ...
                <Button variant="outline" size="icon-lg" className="rounded-2xl bg-card shadow-md hover:bg-secondary"
                        onClick={() => setPanelOpen((open) => !open)} aria-label="Abrir o cerrar panel lateral">
                    <Filter/>
                </Button>
            </div>
            ...
            {toastMessage && (
                <div
                    className="pointer-events-none fixed top-20 left-1/2 z-40 flex -translate-x-1/2 items-center gap-2 rounded-full bg-primary/95 px-4 py-2 text-xs font-semibold text-primary-foreground shadow-xl backdrop-blur-md animate-toast-slide">
...
function Metric({value, label, alert = false}: { value: string; label: string; alert?: boolean }) {
    return <div className="rounded-2xl bg-secondary/75 p-3"><p
        className={cn('text-xl font-bold', alert && 'text-destructive')}>{value}</p><p
        className="mt-1 text-[11px] text-muted-foreground">{label}</p></div>
}
...
function ReportRow({report, active, onClick}: { report: ReporteResponse; active: boolean; onClick: () => void }) {
    ...
    return (
        <button onClick={onClick}
                className={cn('flex items-center gap-3 rounded-2xl border p-3 text-left transition-colors', active ? 'border-primary/30 bg-primary/10' : 'border-transparent bg-secondary/60 hover:bg-secondary')}>
            <div
                className={cn('flex size-9 shrink-0 items-center justify-center rounded-xl', critical ? 'bg-destructive/15 text-destructive' : validated ? 'bg-primary/15 text-primary' : 'bg-accent text-accent-foreground')}>
                {validated ? <CheckCircle2 aria-hidden="true"/> : critical ? <CircleAlert aria-hidden="true"/> :
                    <ShieldCheck aria-hidden="true"/>}
            </div>
            <div className="min-w-0 flex-1"><p className="truncate text-sm font-semibold">{location}</p><p
                className="mt-0.5 text-xs text-muted-foreground">{report.severidad} · {report.totalValidaciones} likes</p>
            </div>
